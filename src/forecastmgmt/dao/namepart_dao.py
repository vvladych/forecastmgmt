...
class NamepartDAO:
    ...
    def __init__(self):
        self.sql_select_dict={"get_nameparts_for_person_sid":"SELECT sid, common_name, name_part_role, name_part_value FROM fc_person_name_part WHERE person_sid=%s"}
        self.sql_insert_dict={"insert_namepart":"INSERT INTO fc_person_name_part(name_part_role, name_part_value, person_name_sid) VALUES(%s, %s, %s) RETURNING sid"}
    ...
    def insert_namepart(self, namepart):
        cur = get_db_connection().cursor()
        cur.execute(self.sql_insert_dict["insert_namepart"],(namepart.namepart_role, namepart.namepart_value, namepart.person_name_sid,))
        namepart.sid = cur.fetchone()[0]
