...
class Publication(MDO):
    ...
    sql_dict={"get_all":"SELECT sid, publisher_sid, uuid, publishing_date, title FROM fc_publication",
              ...
              "insert":"INSERT INTO fc_publication(publisher_sid,publishing_date,title,publication_text) VALUES(%s,%s,%s,%s) RETURNING sid",
              "load":"SELECT title, uuid FROM fc_publication WHERE sid=%s"}
    
    def __init__(self, sid=None, uuid=None, publisher_sid=None, publishing_date=None, title=None, publication_text=None):
    ...
    def get_insert_data(self):
        return (self.publisher_sid,self.publishing_date,self.title,self.publication_text,)
    
    def fabric_method(self,rec):
        return Publication(rec.sid, rec.uuid, rec.publisher_sid, rec.publisher_date, rec.title, rec.publication_text)
